...
/* FILTERING & SEARCHING & SORTING & PAGINATION */
    ...
    // Veri çekme işlemi bir kez yapılıyor
    const getData = async (Model, customFilter = {}) => {
        return await Model.find({ ...filter, ...search, ...customFilter })
    }

    // Run for output:
    res.getModelList = async (Model, customFilter = {}, populate = null) => {
        const data = await getData(Model, customFilter)
        return await data.sort(sort).skip(skip).limit(limit).populate(populate)
    }

    // Details:
    res.getModelListDetails = async (Model, customFilter = {}) => {
        const data = await getData(Model, customFilter)
        console.log("datamız", data.length)
