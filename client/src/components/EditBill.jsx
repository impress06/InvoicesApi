...
import InvoiceItem from "./InvoicesItem";
...
const InvoiceForm = () => {
  ...
  const { updateInvoices } = useAuthApiCall();

  const { bill } = useSelector((state) => state.bill);
  const bills = bill.find((bill) => bill._id === id);
  ...
  const [billFrom, setBillFrom] = useState("Hattat Yazılım");
  const [billFromEmail, setBillFromEmail] = useState("info@hattat.com");
  const [billFromAddress, setBillFromAddress] = useState("Kayseri/Türkiye");
  ...
  // Bill bilgilerini form state'ine geçirmek için useEffect kullanımı
  useEffect(() => {
    if (bills) {
      setBillTo(bills.clientname);
      setBillToEmail(bills.usermail);
      setBillToAddress(bills.address);
      setDueDate(bills.dueDate);
      setItems(
        bills.items.map((item) => ({
          ...item,
          price: item.price.toString(), // Price'ı string'e çeviriyoruz
          quantity: item.quantity.toString(), // Quantity'i string'e çeviriyoruz
        }))
      );
    }
  }, [bills]);
  ...
  const handleClick = () => {
    ...
    console.log(payload);
    updateInvoices("bill", id, payload);
  };
